...
class PretrainedWav2VecModel(nn.Module):
    def __init__(self, prompt_len=0, map_num=5, n_class=16, reprog_front=None, is_cuda=True):
        ...
        self.feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(
            "facebook/wav2vec2-base-960h", return_attention_mask=True
        )
    ...
    def forward(self, x):
        ...
        x = [xx.detach().cpu().numpy() for xx in x]

        # ✅ Ensure no NaN or Inf in input waveform
        cleaned = []
        for i, xx in enumerate(x):
            if np.isnan(xx).any() or np.isinf(xx).any():
                print(f"[WARNING] NaN or Inf detected in input #{i}, replacing with zeros")
                xx = np.nan_to_num(xx, nan=0.0, posinf=0.0, neginf=0.0)
            if len(xx) < 1000:
                print(f"[WARNING] input #{i} too short, padding to minimum length")
                xx = np.pad(xx, (0, 1000 - len(xx)), 'constant')
            cleaned.append(xx.astype(np.float32))

        x = cleaned  # List[np.ndarray] of float32
        inputs = self.feature_extractor(
            x, sampling_rate=16000, return_tensors="pt", padding=True
        )

        input_values = inputs.input_values.to(x_device)
        attention_mask = inputs.attention_mask.to(x_device)
