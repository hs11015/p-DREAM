...
class PretrainedWhisperModel(nn.Module):
    def __init__(self, prompt_len=0, map_num=5, n_class=16, reprog_front=None, is_cuda=True):
        super().__init__()
        # Load encoder-only Whisper model
        self.feature_extractor = WhisperFeatureExtractor.from_pretrained("openai/whisper-base")
        self.model = WhisperModel.from_pretrained("openai/whisper-base")  # use encoder-only version
        # Load encoder-only Whisper model
        # self.feature_extractor = WhisperFeatureExtractor.from_pretrained("openai/whisper-large-v3")
        # self.model = WhisperModel.from_pretrained("openai/whisper-large-v3")  # use encoder-only version
        ...
        #sd, cfg, task = fairseq.checkpoint_utils.load_model_ensemble_and_task(['/workspace/hyunseo/DAIC/pretrained_models/wav2vec_small_960h.pt'])
        #model.load_state_dict(sd)
        
        #model, cfg, task = fairseq.checkpoint_utils.load_model_ensemble_and_task(['/workspace/hyunseo/DAIC/pretrained_models/wav2vec_small_960h.pt'])
        #model = model[0]
        #model.eval()
        ...
        summary(self.model)
        # self.linear = nn.Linear(160000+(16000*prompt_len),n_class)
        """ BASE SIZE """
    ...
    '''def forward(self, x):
        with torch.no_grad():
            z = self.linear.feature_extractor(x)
            if isinstance(z, tuple):
                z = z[0]
            c = self.linear.feature_aggregator(z)
        return z, c'''
    
    def forward(self, x):
        assert x.ndim == 2, f"Expected input of shape (B, T), got {x.shape}"
        x_device = x.device
        x = [xx.cpu().numpy() for xx in x]
        x = self.feature_extractor(x, sampling_rate=16000, return_tensors="pt")["input_features"].to(x_device)
        # print(x.shape)
...
class Prediction:
    ...
    def __init__(self, fname, gpu=0):
        self.gpu = gpu
        self.model = PretrainedWhisperModel(fname).cuda(gpu)
