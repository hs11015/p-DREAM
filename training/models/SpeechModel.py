...
class AttRNNSpeechModel(torch.nn.Module):
    def __init__(self, **kwargs):
        ...
        for name, param in self.skip_conv.named_parameters():
            param.requires_grad = False
        for name, param in self.skip_linear.named_parameters():
            param.requires_grad = False
        for name, param in self.conv_block.named_parameters():
            param.requires_grad = False
        for name, param in self.lstm_block.named_parameters():
            param.requires_grad = False
...
class V2SReprogModel(torch.nn.Module):
    def __init__(self, map_num=5, n_class=16, reprog_front=None, is_cuda=True):
        ...
        # self.lmbd = args.lmbd
        self.linear = nn.Linear(1155, 100)
    ...
    def load(self):
        ...
        with h5py.File('/workspace/hyunseo/DAIC/pretrained_models/model-attRNN.h5', 'r') as f:
            args = f["model_weights"].keys()

            model_dict = model.state_dict()
            print(model_dict)
            pretrained_dict = {k: v for k, v in f["model_weights"].keys() if k in model_dict}
