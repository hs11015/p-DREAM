...
class ASTModel(nn.Module):
    ...
    def forward_features(self, x):
        """
        Extract [CLS] token embedding from input spectrogram.
        """
        x = x.unsqueeze(1)         # (B, 1, T, F)
        x = x.transpose(2, 3)      # (B, 1, F, T)

        B = x.shape[0]
        x = self.v.patch_embed(x)  # (B, N, D)
        ...
        for blk in self.v.blocks:
            x = blk(x)
        x = self.v.norm(x)
        x = (x[:, 0] + x[:, 1]) / 2  # average [CLS] and [DIST] token
        return x  # [B, D]
    ...
    def forward(self, x, skip=None):
        ...
        x = x.transpose(2, 3)
        '''
        if self.count >= 1200*4:
            D_octave = np.abs(x[0][0].cpu())
            librosa.display.specshow(librosa.amplitude_to_db(D_octave, ref=np.max))#, sr=16000, y_axis='linear', x_axis='time')
            #plt.plot(features[0].cpu())
            plt.ylabel('Frequency')
            plt.xlabel('Time')
            plt.ylim(0, 128)
            plt.xlim(0, 540)
            plt.savefig(f'./spectrogram/{self.count}.png')
            print(self.count)
        self.count += 1
        '''
...
class AST(torch.nn.Module):
    def __init__(self, map_num=5, n_class=16, reprog_front=None, is_cuda=True):
        super().__init__()
    ...
    def forward(self, wav):
        ...
        if self.reprog_front == "skip":
            # print(f"linear input : {self.linear}, {self.linear.shape}")
            ...
            emb = self.ast_mdl.forward_features(features)  # [B, 768]
            predicted = self.linear(emb)                  # [B, n_class]
